...
abstract class _$BooleanOperationNodeCWProxy {
  BooleanOperationNode id(String id);

  BooleanOperationNode name(String name);

  BooleanOperationNode visible(bool visible);

  BooleanOperationNode locked(bool locked);

  BooleanOperationNode scrollBehavior(ScrollBehavior scrollBehavior);

  BooleanOperationNode rotation(num rotation);

  BooleanOperationNode componentPropertyReferences(
    Map<String, String> componentPropertyReferences,
  );

  BooleanOperationNode pluginData(Object? pluginData);

  BooleanOperationNode sharedPluginData(Object? sharedPluginData);

  BooleanOperationNode boundVariables(LayerTraitVariables boundVariables);

  BooleanOperationNode explicitVariableModes(
    Map<String, String> explicitVariableModes,
  );

  BooleanOperationNode blendMode(BlendMode blendMode);

  BooleanOperationNode opacity(num opacity);

  BooleanOperationNode children(List<SubCanvasNode> children);

  BooleanOperationNode absoluteBoundingBox(Rectangle? absoluteBoundingBox);

  BooleanOperationNode absoluteRenderBounds(Rectangle? absoluteRenderBounds);

  BooleanOperationNode preserveRatio(bool preserveRatio);

  BooleanOperationNode constraints(LayoutConstraint? constraints);

  BooleanOperationNode relativeTransform(Transform? relativeTransform);

  BooleanOperationNode size(Vector? size);

  BooleanOperationNode layoutAlign(LayoutAlign? layoutAlign);

  BooleanOperationNode layoutGrow(LayoutGrow layoutGrow);

  BooleanOperationNode layoutPositioning(LayoutPositioning layoutPositioning);

  BooleanOperationNode minWidth(num minWidth);

  BooleanOperationNode maxWidth(num maxWidth);

  BooleanOperationNode minHeight(num minHeight);

  BooleanOperationNode maxHeight(num maxHeight);

  BooleanOperationNode layoutSizingHorizontal(
    LayoutSizing? layoutSizingHorizontal,
  );

  BooleanOperationNode layoutSizingVertical(LayoutSizing? layoutSizingVertical);

  BooleanOperationNode gridRowCount(num? gridRowCount);

  BooleanOperationNode gridColumnCount(num? gridColumnCount);

  BooleanOperationNode gridRowGap(num gridRowGap);

  BooleanOperationNode gridColumnGap(num gridColumnGap);

  BooleanOperationNode gridColumnsSizing(String? gridColumnsSizing);

  BooleanOperationNode gridRowsSizing(String? gridRowsSizing);

  BooleanOperationNode gridChildHorizontalAlign(
    GridChildAlign? gridChildHorizontalAlign,
  );

  BooleanOperationNode gridChildVerticalAlign(
    GridChildAlign? gridChildVerticalAlign,
  );

  BooleanOperationNode gridRowSpan(num gridRowSpan);

  BooleanOperationNode gridColumnSpan(num gridColumnSpan);

  BooleanOperationNode gridRowAnchorIndex(num gridRowAnchorIndex);

  BooleanOperationNode gridColumnAnchorIndex(num gridColumnAnchorIndex);

  BooleanOperationNode fillOverrideTable(
    Map<String, PaintOverride?> fillOverrideTable,
  );

  BooleanOperationNode fillGeometry(List<Path> fillGeometry);

  BooleanOperationNode strokeGeometry(List<Path> strokeGeometry);

  BooleanOperationNode strokeCap(StrokeCap strokeCap);

  BooleanOperationNode strokeMiterAngle(num strokeMiterAngle);

  BooleanOperationNode fills(List<Paint> fills);

  BooleanOperationNode styles(Map<String, String> styles);

  BooleanOperationNode strokes(List<Paint> strokes);

  BooleanOperationNode strokeWeight(num strokeWeight);

  BooleanOperationNode strokeAlign(StrokeAlign? strokeAlign);

  BooleanOperationNode strokeJoin(StrokeJoin strokeJoin);

  BooleanOperationNode strokeDashes(List<num> strokeDashes);

  BooleanOperationNode exportSettings(List<ExportSetting> exportSettings);

  BooleanOperationNode effects(List<Effect> effects);

  BooleanOperationNode isMask(bool isMask);

  BooleanOperationNode maskType(MaskType? maskType);

  BooleanOperationNode transitionNodeId(String? transitionNodeId);

  BooleanOperationNode transitionDuration(num? transitionDuration);

  BooleanOperationNode transitionEasing(EasingType? transitionEasing);

  BooleanOperationNode interactions(List<Interaction> interactions);
...
/// Use as `instanceOfBooleanOperationNode.copyWith(...)` or call `instanceOfBooleanOperationNode.copyWith.fieldName(value)` for a single field.
class _$BooleanOperationNodeCWProxyImpl
    implements _$BooleanOperationNodeCWProxy {
  const _$BooleanOperationNodeCWProxyImpl(this._value);

  final BooleanOperationNode _value;

  @override
  BooleanOperationNode id(String id) => call(id: id);

  @override
  BooleanOperationNode name(String name) => call(name: name);

  @override
  BooleanOperationNode visible(bool visible) => call(visible: visible);

  @override
  BooleanOperationNode locked(bool locked) => call(locked: locked);
  ...
  @override
  BooleanOperationNode rotation(num rotation) => call(rotation: rotation);
  ...
  @override
  BooleanOperationNode opacity(num opacity) => call(opacity: opacity);
  ...
  @override
  BooleanOperationNode size(Vector? size) => call(size: size);
  ...
  @override
  BooleanOperationNode minWidth(num minWidth) => call(minWidth: minWidth);

  @override
  BooleanOperationNode maxWidth(num maxWidth) => call(maxWidth: maxWidth);

  @override
  BooleanOperationNode minHeight(num minHeight) => call(minHeight: minHeight);

  @override
  BooleanOperationNode maxHeight(num maxHeight) => call(maxHeight: maxHeight);
  ...
  @override
  BooleanOperationNode layoutSizingVertical(
    LayoutSizing? layoutSizingVertical,
  ...
  @override
  BooleanOperationNode fills(List<Paint> fills) => call(fills: fills);
  ...
  @override
  BooleanOperationNode strokes(List<Paint> strokes) => call(strokes: strokes);
  ...
  @override
  BooleanOperationNode effects(List<Effect> effects) => call(effects: effects);

  @override
  BooleanOperationNode isMask(bool isMask) => call(isMask: isMask);

  @override
  BooleanOperationNode maskType(MaskType? maskType) => call(maskType: maskType);
  ...
  BooleanOperationNode call({
    ...
    return BooleanOperationNode(
      ...
      booleanOperation:
          booleanOperation == const $CopyWithPlaceholder() ||
              booleanOperation == null
          ? _value.booleanOperation
          // ignore: cast_nullable_to_non_nullable
          : booleanOperation as BooleanOperation,
    );
  }
}

extension $BooleanOperationNodeCopyWith on BooleanOperationNode {
  /// Returns a callable class used to build a new instance with modified fields.
  /// Example: `instanceOfBooleanOperationNode.copyWith(...)` or `instanceOfBooleanOperationNode.copyWith.fieldName(...)`.
  // ignore: library_private_types_in_public_api
  _$BooleanOperationNodeCWProxy get copyWith =>
      _$BooleanOperationNodeCWProxyImpl(this);
}
...
BooleanOperationNode _$BooleanOperationNodeFromJson(
  ...
  interactions:
      (json['interactions'] as List<dynamic>?)
          ...
          .toList() ??
      [],
  booleanOperation: $enumDecode(
    _$BooleanOperationEnumMap,
...
Map<String, dynamic> _$BooleanOperationNodeToJson(
  ...
  'booleanOperation': _$BooleanOperationEnumMap[instance.booleanOperation]!,
