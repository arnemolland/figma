...
part of 'transform_group_node.dart';

// **************************************************************************
// CopyWithGenerator
// **************************************************************************

abstract class _$TransformGroupNodeCWProxy {
  TransformGroupNode id(String id);

  TransformGroupNode name(String name);

  TransformGroupNode visible(bool visible);

  TransformGroupNode locked(bool locked);

  TransformGroupNode scrollBehavior(ScrollBehavior scrollBehavior);

  TransformGroupNode rotation(num rotation);

  TransformGroupNode componentPropertyReferences(
    Map<String, String> componentPropertyReferences,
  );

  TransformGroupNode pluginData(Object? pluginData);

  TransformGroupNode sharedPluginData(Object? sharedPluginData);

  TransformGroupNode boundVariables(LayerTraitVariables boundVariables);

  TransformGroupNode explicitVariableModes(
    Map<String, String> explicitVariableModes,
  );

  TransformGroupNode blendMode(BlendMode blendMode);

  TransformGroupNode opacity(num opacity);

  TransformGroupNode children(List<SubCanvasNode> children);

  TransformGroupNode absoluteBoundingBox(Rectangle? absoluteBoundingBox);

  TransformGroupNode absoluteRenderBounds(Rectangle? absoluteRenderBounds);

  TransformGroupNode preserveRatio(bool preserveRatio);

  TransformGroupNode constraints(LayoutConstraint? constraints);

  TransformGroupNode relativeTransform(Transform? relativeTransform);

  TransformGroupNode size(Vector? size);

  TransformGroupNode layoutAlign(LayoutAlign? layoutAlign);

  TransformGroupNode layoutGrow(LayoutGrow layoutGrow);

  TransformGroupNode layoutPositioning(LayoutPositioning layoutPositioning);

  TransformGroupNode minWidth(num minWidth);

  TransformGroupNode maxWidth(num maxWidth);

  TransformGroupNode minHeight(num minHeight);

  TransformGroupNode maxHeight(num maxHeight);

  TransformGroupNode layoutSizingHorizontal(
    LayoutSizing? layoutSizingHorizontal,
  );

  TransformGroupNode layoutSizingVertical(LayoutSizing? layoutSizingVertical);

  TransformGroupNode gridRowCount(num? gridRowCount);

  TransformGroupNode gridColumnCount(num? gridColumnCount);

  TransformGroupNode gridRowGap(num gridRowGap);

  TransformGroupNode gridColumnGap(num gridColumnGap);

  TransformGroupNode gridColumnsSizing(String? gridColumnsSizing);

  TransformGroupNode gridRowsSizing(String? gridRowsSizing);

  TransformGroupNode gridChildHorizontalAlign(
    GridChildAlign? gridChildHorizontalAlign,
  );

  TransformGroupNode gridChildVerticalAlign(
    GridChildAlign? gridChildVerticalAlign,
  );

  TransformGroupNode gridRowSpan(num gridRowSpan);

  TransformGroupNode gridColumnSpan(num gridColumnSpan);

  TransformGroupNode gridRowAnchorIndex(num gridRowAnchorIndex);

  TransformGroupNode gridColumnAnchorIndex(num gridColumnAnchorIndex);

  TransformGroupNode clipsContent(bool clipsContent);

  TransformGroupNode layoutGrids(List<LayoutGrid> layoutGrids);

  TransformGroupNode overflowDirection(OverflowDirection overflowDirection);

  TransformGroupNode layoutMode(LayoutMode layoutMode);

  TransformGroupNode primaryAxisSizingMode(
    PrimaryAxisSizingMode primaryAxisSizingMode,
  );

  TransformGroupNode counterAxisSizingMode(
    CounterAxisSizingMode counterAxisSizingMode,
  );

  TransformGroupNode primaryAxisAlignItems(
    PrimaryAxisAlignItems primaryAxisAlignItems,
  );

  TransformGroupNode counterAxisAlignItems(
    CounterAxisAlignItems counterAxisAlignItems,
  );

  TransformGroupNode paddingLeft(num paddingLeft);

  TransformGroupNode paddingRight(num paddingRight);

  TransformGroupNode paddingTop(num paddingTop);

  TransformGroupNode paddingBottom(num paddingBottom);

  TransformGroupNode itemSpacing(num itemSpacing);

  TransformGroupNode itemReverseZIndex(bool itemReverseZIndex);

  TransformGroupNode strokesIncludedInLayout(bool strokesIncludedInLayout);

  TransformGroupNode layoutWrap(LayoutWrap? layoutWrap);

  TransformGroupNode counterAxisSpacing(num? counterAxisSpacing);

  TransformGroupNode counterAxisAlignContent(
    CounterAxisAlignContent counterAxisAlignContent,
  );

  TransformGroupNode cornerRadius(num cornerRadius);

  TransformGroupNode cornerSmoothing(num cornerSmoothing);

  TransformGroupNode rectangleCornerRadii(List<num> rectangleCornerRadii);

  TransformGroupNode fills(List<Paint> fills);

  TransformGroupNode styles(Map<String, String> styles);

  TransformGroupNode strokes(List<Paint> strokes);

  TransformGroupNode strokeWeight(num strokeWeight);

  TransformGroupNode strokeAlign(StrokeAlign? strokeAlign);

  TransformGroupNode strokeJoin(StrokeJoin strokeJoin);

  TransformGroupNode strokeDashes(List<num> strokeDashes);

  TransformGroupNode fillOverrideTable(
    Map<String, PaintOverride?> fillOverrideTable,
  );

  TransformGroupNode fillGeometry(List<Path> fillGeometry);

  TransformGroupNode strokeGeometry(List<Path> strokeGeometry);

  TransformGroupNode strokeCap(StrokeCap strokeCap);

  TransformGroupNode strokeMiterAngle(num strokeMiterAngle);

  TransformGroupNode exportSettings(List<ExportSetting> exportSettings);

  TransformGroupNode effects(List<Effect> effects);

  TransformGroupNode isMask(bool isMask);

  TransformGroupNode maskType(MaskType? maskType);

  TransformGroupNode transitionNodeId(String? transitionNodeId);

  TransformGroupNode transitionDuration(num? transitionDuration);

  TransformGroupNode transitionEasing(EasingType? transitionEasing);

  TransformGroupNode interactions(List<Interaction> interactions);

  TransformGroupNode individualStrokeWeights(
    StrokeWeights? individualStrokeWeights,
  );

  TransformGroupNode devStatus(DevStatus? devStatus);

  /// Creates a new instance with the provided field values.
  /// Passing `null` to a nullable field nullifies it, while `null` for a non-nullable field is ignored. To update a single field use `TransformGroupNode(...).copyWith.fieldName(value)`.
  ///
  /// Example:
  /// ```dart
  /// TransformGroupNode(...).copyWith(id: 12, name: "My name")
  /// ```
  TransformGroupNode call({
    String id,
    String name,
    bool visible,
    bool locked,
...
/// Use as `instanceOfTransformGroupNode.copyWith(...)` or call `instanceOfTransformGroupNode.copyWith.fieldName(value)` for a single field.
class _$TransformGroupNodeCWProxyImpl implements _$TransformGroupNodeCWProxy {
  const _$TransformGroupNodeCWProxyImpl(this._value);

  final TransformGroupNode _value;

  @override
  TransformGroupNode id(String id) => call(id: id);

  @override
  TransformGroupNode name(String name) => call(name: name);

  @override
  TransformGroupNode visible(bool visible) => call(visible: visible);

  @override
  TransformGroupNode locked(bool locked) => call(locked: locked);
  ...
  @override
  TransformGroupNode rotation(num rotation) => call(rotation: rotation);
  ...
  @override
  TransformGroupNode opacity(num opacity) => call(opacity: opacity);
  ...
  @override
  TransformGroupNode size(Vector? size) => call(size: size);
  ...
  @override
  TransformGroupNode minWidth(num minWidth) => call(minWidth: minWidth);

  @override
  TransformGroupNode maxWidth(num maxWidth) => call(maxWidth: maxWidth);

  @override
  TransformGroupNode minHeight(num minHeight) => call(minHeight: minHeight);

  @override
  TransformGroupNode maxHeight(num maxHeight) => call(maxHeight: maxHeight);
  ...
  @override
  TransformGroupNode gridRowGap(num gridRowGap) => call(gridRowGap: gridRowGap);
  ...
  @override
  TransformGroupNode paddingTop(num paddingTop) => call(paddingTop: paddingTop);
  ...
  @override
  TransformGroupNode fills(List<Paint> fills) => call(fills: fills);

  @override
  TransformGroupNode styles(Map<String, String> styles) => call(styles: styles);

  @override
  TransformGroupNode strokes(List<Paint> strokes) => call(strokes: strokes);
  ...
  @override
  TransformGroupNode effects(List<Effect> effects) => call(effects: effects);

  @override
  TransformGroupNode isMask(bool isMask) => call(isMask: isMask);

  @override
  TransformGroupNode maskType(MaskType? maskType) => call(maskType: maskType);
...
extension $TransformGroupNodeCopyWith on TransformGroupNode {
  /// Returns a callable class used to build a new instance with modified fields.
  /// Example: `instanceOfTransformGroupNode.copyWith(...)` or `instanceOfTransformGroupNode.copyWith.fieldName(...)`.
  // ignore: library_private_types_in_public_api
  _$TransformGroupNodeCWProxy get copyWith =>
      _$TransformGroupNodeCWProxyImpl(this);
}
...
TransformGroupNode _$TransformGroupNodeFromJson(
...
Map<String, dynamic> _$TransformGroupNodeToJson(
  TransformGroupNode instance,
) => <String, dynamic>{
  'id': instance.id,
  'name': instance.name,
