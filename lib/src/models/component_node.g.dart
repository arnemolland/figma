...
abstract class _$ComponentNodeCWProxy {
  ...
  ComponentNode scrollBehavior(ScrollBehavior scrollBehavior);

  ComponentNode rotation(num rotation);

  ComponentNode componentPropertyReferences(
    Map<String, String> componentPropertyReferences,
  );

  ComponentNode pluginData(Object? pluginData);

  ComponentNode sharedPluginData(Object? sharedPluginData);

  ComponentNode boundVariables(LayerTraitVariables boundVariables);

  ComponentNode explicitVariableModes(
    Map<String, String> explicitVariableModes,
  );

  ComponentNode blendMode(BlendMode blendMode);

  ComponentNode opacity(num opacity);

  ComponentNode children(List<SubCanvasNode> children);

  ComponentNode absoluteBoundingBox(Rectangle? absoluteBoundingBox);

  ComponentNode absoluteRenderBounds(Rectangle? absoluteRenderBounds);

  ComponentNode preserveRatio(bool preserveRatio);

  ComponentNode constraints(LayoutConstraint? constraints);

  ComponentNode relativeTransform(Transform? relativeTransform);

  ComponentNode size(Vector? size);

  ComponentNode layoutAlign(LayoutAlign? layoutAlign);

  ComponentNode layoutGrow(LayoutGrow layoutGrow);

  ComponentNode layoutPositioning(LayoutPositioning layoutPositioning);

  ComponentNode minWidth(num minWidth);

  ComponentNode maxWidth(num maxWidth);

  ComponentNode minHeight(num minHeight);

  ComponentNode maxHeight(num maxHeight);

  ComponentNode layoutSizingHorizontal(LayoutSizing? layoutSizingHorizontal);

  ComponentNode layoutSizingVertical(LayoutSizing? layoutSizingVertical);

  ComponentNode gridRowCount(num? gridRowCount);

  ComponentNode gridColumnCount(num? gridColumnCount);

  ComponentNode gridRowGap(num gridRowGap);

  ComponentNode gridColumnGap(num gridColumnGap);

  ComponentNode gridColumnsSizing(String? gridColumnsSizing);

  ComponentNode gridRowsSizing(String? gridRowsSizing);

  ComponentNode gridChildHorizontalAlign(
    GridChildAlign? gridChildHorizontalAlign,
  );

  ComponentNode gridChildVerticalAlign(GridChildAlign? gridChildVerticalAlign);

  ComponentNode gridRowSpan(num gridRowSpan);

  ComponentNode gridColumnSpan(num gridColumnSpan);

  ComponentNode gridRowAnchorIndex(num gridRowAnchorIndex);

  ComponentNode gridColumnAnchorIndex(num gridColumnAnchorIndex);
  ...
  ComponentNode paddingLeft(num paddingLeft);

  ComponentNode paddingRight(num paddingRight);

  ComponentNode paddingTop(num paddingTop);

  ComponentNode paddingBottom(num paddingBottom);

  ComponentNode itemSpacing(num itemSpacing);
  ...
  ComponentNode counterAxisSpacing(num? counterAxisSpacing);

  ComponentNode counterAxisAlignContent(
    CounterAxisAlignContent counterAxisAlignContent,
  );

  ComponentNode cornerRadius(num cornerRadius);

  ComponentNode cornerSmoothing(num cornerSmoothing);

  ComponentNode rectangleCornerRadii(List<num> rectangleCornerRadii);

  ComponentNode fills(List<Paint> fills);

  ComponentNode styles(Map<String, String> styles);

  ComponentNode strokes(List<Paint> strokes);

  ComponentNode strokeWeight(num strokeWeight);

  ComponentNode strokeAlign(StrokeAlign? strokeAlign);

  ComponentNode strokeJoin(StrokeJoin strokeJoin);

  ComponentNode strokeDashes(List<num> strokeDashes);

  ComponentNode fillOverrideTable(
    Map<String, PaintOverride?> fillOverrideTable,
  );

  ComponentNode fillGeometry(List<Path> fillGeometry);

  ComponentNode strokeGeometry(List<Path> strokeGeometry);

  ComponentNode strokeCap(StrokeCap strokeCap);

  ComponentNode strokeMiterAngle(num strokeMiterAngle);
  ...
  ComponentNode maskType(MaskType? maskType);

  ComponentNode transitionNodeId(String? transitionNodeId);

  ComponentNode transitionDuration(num? transitionDuration);

  ComponentNode transitionEasing(EasingType? transitionEasing);

  ComponentNode interactions(List<Interaction> interactions);
  ...
  /// Passing `null` to a nullable field nullifies it, while `null` for a non-nullable field is ignored. To update a single field use `ComponentNode(...).copyWith.fieldName(value)`.
  ///
  /// Example:
  /// ```dart
...
/// Use as `instanceOfComponentNode.copyWith(...)` or call `instanceOfComponentNode.copyWith.fieldName(value)` for a single field.
class _$ComponentNodeCWProxyImpl implements _$ComponentNodeCWProxy {
  ...
  @override
  ComponentNode id(String id) => call(id: id);

  @override
  ComponentNode name(String name) => call(name: name);

  @override
  ComponentNode visible(bool visible) => call(visible: visible);

  @override
  ComponentNode locked(bool locked) => call(locked: locked);
  ...
  @override
  ComponentNode rotation(num rotation) => call(rotation: rotation);
  ...
  @override
  ComponentNode pluginData(Object? pluginData) => call(pluginData: pluginData);
  ...
  @override
  ComponentNode blendMode(BlendMode blendMode) => call(blendMode: blendMode);

  @override
  ComponentNode opacity(num opacity) => call(opacity: opacity);
  ...
  @override
  ComponentNode size(Vector? size) => call(size: size);
  ...
  @override
  ComponentNode minWidth(num minWidth) => call(minWidth: minWidth);

  @override
  ComponentNode maxWidth(num maxWidth) => call(maxWidth: maxWidth);

  @override
  ComponentNode minHeight(num minHeight) => call(minHeight: minHeight);

  @override
  ComponentNode maxHeight(num maxHeight) => call(maxHeight: maxHeight);
  ...
  @override
  ComponentNode gridRowGap(num gridRowGap) => call(gridRowGap: gridRowGap);
  ...
  @override
  ComponentNode gridChildVerticalAlign(
    GridChildAlign? gridChildVerticalAlign,
  ) => call(gridChildVerticalAlign: gridChildVerticalAlign);

  @override
  ComponentNode gridRowSpan(num gridRowSpan) => call(gridRowSpan: gridRowSpan);
  ...
  @override
  ComponentNode paddingLeft(num paddingLeft) => call(paddingLeft: paddingLeft);
  ...
  @override
  ComponentNode paddingTop(num paddingTop) => call(paddingTop: paddingTop);
  ...
  @override
  ComponentNode itemSpacing(num itemSpacing) => call(itemSpacing: itemSpacing);
  ...
  @override
  ComponentNode fills(List<Paint> fills) => call(fills: fills);

  @override
  ComponentNode styles(Map<String, String> styles) => call(styles: styles);

  @override
  ComponentNode strokes(List<Paint> strokes) => call(strokes: strokes);
  ...
  @override
  ComponentNode strokeCap(StrokeCap strokeCap) => call(strokeCap: strokeCap);
  ...
  @override
  ComponentNode effects(List<Effect> effects) => call(effects: effects);

  @override
  ComponentNode isMask(bool isMask) => call(isMask: isMask);

  @override
  ComponentNode maskType(MaskType? maskType) => call(maskType: maskType);
  ...
  @override
  ComponentNode devStatus(DevStatus? devStatus) => call(devStatus: devStatus);
...
extension $ComponentNodeCopyWith on ComponentNode {
  ...
  /// Example: `instanceOfComponentNode.copyWith(...)` or `instanceOfComponentNode.copyWith.fieldName(...)`.
