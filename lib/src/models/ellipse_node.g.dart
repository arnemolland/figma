...
part of 'ellipse_node.dart';

// **************************************************************************
// CopyWithGenerator
// **************************************************************************

abstract class _$EllipseNodeCWProxy {
  EllipseNode id(String id);

  EllipseNode name(String name);

  EllipseNode visible(bool visible);

  EllipseNode locked(bool locked);

  EllipseNode scrollBehavior(ScrollBehavior scrollBehavior);

  EllipseNode rotation(num rotation);

  EllipseNode componentPropertyReferences(
    Map<String, String> componentPropertyReferences,
  );

  EllipseNode pluginData(Object? pluginData);

  EllipseNode sharedPluginData(Object? sharedPluginData);

  EllipseNode boundVariables(LayerTraitVariables boundVariables);

  EllipseNode explicitVariableModes(Map<String, String> explicitVariableModes);

  EllipseNode blendMode(BlendMode blendMode);

  EllipseNode opacity(num opacity);

  EllipseNode absoluteBoundingBox(Rectangle? absoluteBoundingBox);

  EllipseNode absoluteRenderBounds(Rectangle? absoluteRenderBounds);

  EllipseNode preserveRatio(bool preserveRatio);

  EllipseNode constraints(LayoutConstraint? constraints);

  EllipseNode relativeTransform(Transform? relativeTransform);

  EllipseNode size(Vector? size);

  EllipseNode layoutAlign(LayoutAlign? layoutAlign);

  EllipseNode layoutGrow(LayoutGrow layoutGrow);

  EllipseNode layoutPositioning(LayoutPositioning layoutPositioning);

  EllipseNode minWidth(num minWidth);

  EllipseNode maxWidth(num maxWidth);

  EllipseNode minHeight(num minHeight);

  EllipseNode maxHeight(num maxHeight);

  EllipseNode layoutSizingHorizontal(LayoutSizing? layoutSizingHorizontal);

  EllipseNode layoutSizingVertical(LayoutSizing? layoutSizingVertical);

  EllipseNode gridRowCount(num? gridRowCount);

  EllipseNode gridColumnCount(num? gridColumnCount);

  EllipseNode gridRowGap(num gridRowGap);

  EllipseNode gridColumnGap(num gridColumnGap);

  EllipseNode gridColumnsSizing(String? gridColumnsSizing);

  EllipseNode gridRowsSizing(String? gridRowsSizing);

  EllipseNode gridChildHorizontalAlign(
    GridChildAlign? gridChildHorizontalAlign,
  );

  EllipseNode gridChildVerticalAlign(GridChildAlign? gridChildVerticalAlign);

  EllipseNode gridRowSpan(num gridRowSpan);

  EllipseNode gridColumnSpan(num gridColumnSpan);

  EllipseNode gridRowAnchorIndex(num gridRowAnchorIndex);

  EllipseNode gridColumnAnchorIndex(num gridColumnAnchorIndex);

  EllipseNode fills(List<Paint> fills);

  EllipseNode styles(Map<String, String> styles);

  EllipseNode strokes(List<Paint> strokes);

  EllipseNode strokeWeight(num strokeWeight);

  EllipseNode strokeAlign(StrokeAlign? strokeAlign);

  EllipseNode strokeJoin(StrokeJoin strokeJoin);

  EllipseNode strokeDashes(List<num> strokeDashes);

  EllipseNode fillOverrideTable(Map<String, PaintOverride?> fillOverrideTable);

  EllipseNode fillGeometry(List<Path> fillGeometry);

  EllipseNode strokeGeometry(List<Path> strokeGeometry);

  EllipseNode strokeCap(StrokeCap strokeCap);

  EllipseNode strokeMiterAngle(num strokeMiterAngle);

  EllipseNode exportSettings(List<ExportSetting> exportSettings);

  EllipseNode effects(List<Effect> effects);

  EllipseNode isMask(bool isMask);

  EllipseNode maskType(MaskType? maskType);

  EllipseNode transitionNodeId(String? transitionNodeId);

  EllipseNode transitionDuration(num? transitionDuration);

  EllipseNode transitionEasing(EasingType? transitionEasing);

  EllipseNode interactions(List<Interaction> interactions);

  EllipseNode arcData(ArcData arcData);
...
/// Use as `instanceOfEllipseNode.copyWith(...)` or call `instanceOfEllipseNode.copyWith.fieldName(value)` for a single field.
class _$EllipseNodeCWProxyImpl implements _$EllipseNodeCWProxy {
  const _$EllipseNodeCWProxyImpl(this._value);

  final EllipseNode _value;

  @override
  EllipseNode id(String id) => call(id: id);

  @override
  EllipseNode name(String name) => call(name: name);

  @override
  EllipseNode visible(bool visible) => call(visible: visible);

  @override
  EllipseNode locked(bool locked) => call(locked: locked);
  ...
  @override
  EllipseNode rotation(num rotation) => call(rotation: rotation);
  ...
  @override
  EllipseNode pluginData(Object? pluginData) => call(pluginData: pluginData);
  ...
  @override
  EllipseNode explicitVariableModes(
    Map<String, String> explicitVariableModes,
  ) => call(explicitVariableModes: explicitVariableModes);

  @override
  EllipseNode blendMode(BlendMode blendMode) => call(blendMode: blendMode);

  @override
  EllipseNode opacity(num opacity) => call(opacity: opacity);
  ...
  @override
  EllipseNode size(Vector? size) => call(size: size);
  ...
  @override
  EllipseNode layoutGrow(LayoutGrow layoutGrow) => call(layoutGrow: layoutGrow);
  ...
  @override
  EllipseNode minWidth(num minWidth) => call(minWidth: minWidth);

  @override
  EllipseNode maxWidth(num maxWidth) => call(maxWidth: maxWidth);

  @override
  EllipseNode minHeight(num minHeight) => call(minHeight: minHeight);

  @override
  EllipseNode maxHeight(num maxHeight) => call(maxHeight: maxHeight);
  ...
  @override
  EllipseNode gridRowGap(num gridRowGap) => call(gridRowGap: gridRowGap);
  ...
  @override
  EllipseNode gridRowSpan(num gridRowSpan) => call(gridRowSpan: gridRowSpan);
  ...
  @override
  EllipseNode fills(List<Paint> fills) => call(fills: fills);

  @override
  EllipseNode styles(Map<String, String> styles) => call(styles: styles);

  @override
  EllipseNode strokes(List<Paint> strokes) => call(strokes: strokes);
  ...
  @override
  EllipseNode strokeJoin(StrokeJoin strokeJoin) => call(strokeJoin: strokeJoin);
  ...
  @override
  EllipseNode strokeCap(StrokeCap strokeCap) => call(strokeCap: strokeCap);
  ...
  @override
  EllipseNode effects(List<Effect> effects) => call(effects: effects);

  @override
  EllipseNode isMask(bool isMask) => call(isMask: isMask);

  @override
  EllipseNode maskType(MaskType? maskType) => call(maskType: maskType);
...
extension $EllipseNodeCopyWith on EllipseNode {
  /// Returns a callable class used to build a new instance with modified fields.
  /// Example: `instanceOfEllipseNode.copyWith(...)` or `instanceOfEllipseNode.copyWith.fieldName(...)`.
  // ignore: library_private_types_in_public_api
  _$EllipseNodeCWProxy get copyWith => _$EllipseNodeCWProxyImpl(this);
}
...
EllipseNode _$EllipseNodeFromJson(Map<String, dynamic> json) => EllipseNode(
...
Map<String, dynamic> _$EllipseNodeToJson(
  EllipseNode instance,
) => <String, dynamic>{
  'id': instance.id,
  'name': instance.name,
  ...
  'arcData': instance.arcData.toJson(),
