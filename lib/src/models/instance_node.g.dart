...
abstract class _$InstanceNodeCWProxy {
  InstanceNode id(String id);

  InstanceNode name(String name);

  InstanceNode visible(bool visible);

  InstanceNode locked(bool locked);

  InstanceNode scrollBehavior(ScrollBehavior scrollBehavior);

  InstanceNode rotation(num rotation);

  InstanceNode componentPropertyReferences(
    Map<String, String> componentPropertyReferences,
  );

  InstanceNode pluginData(Object? pluginData);

  InstanceNode sharedPluginData(Object? sharedPluginData);

  InstanceNode boundVariables(LayerTraitVariables boundVariables);

  InstanceNode explicitVariableModes(Map<String, String> explicitVariableModes);

  InstanceNode blendMode(BlendMode blendMode);

  InstanceNode opacity(num opacity);

  InstanceNode children(List<SubCanvasNode> children);

  InstanceNode absoluteBoundingBox(Rectangle? absoluteBoundingBox);

  InstanceNode absoluteRenderBounds(Rectangle? absoluteRenderBounds);

  InstanceNode preserveRatio(bool preserveRatio);

  InstanceNode constraints(LayoutConstraint? constraints);

  InstanceNode relativeTransform(Transform? relativeTransform);

  InstanceNode size(Vector? size);

  InstanceNode layoutAlign(LayoutAlign? layoutAlign);

  InstanceNode layoutGrow(LayoutGrow layoutGrow);

  InstanceNode layoutPositioning(LayoutPositioning layoutPositioning);

  InstanceNode minWidth(num minWidth);

  InstanceNode maxWidth(num maxWidth);

  InstanceNode minHeight(num minHeight);

  InstanceNode maxHeight(num maxHeight);

  InstanceNode layoutSizingHorizontal(LayoutSizing? layoutSizingHorizontal);

  InstanceNode layoutSizingVertical(LayoutSizing? layoutSizingVertical);

  InstanceNode gridRowCount(num? gridRowCount);

  InstanceNode gridColumnCount(num? gridColumnCount);

  InstanceNode gridRowGap(num gridRowGap);

  InstanceNode gridColumnGap(num gridColumnGap);

  InstanceNode gridColumnsSizing(String? gridColumnsSizing);

  InstanceNode gridRowsSizing(String? gridRowsSizing);

  InstanceNode gridChildHorizontalAlign(
    GridChildAlign? gridChildHorizontalAlign,
  );

  InstanceNode gridChildVerticalAlign(GridChildAlign? gridChildVerticalAlign);

  InstanceNode gridRowSpan(num gridRowSpan);

  InstanceNode gridColumnSpan(num gridColumnSpan);

  InstanceNode gridRowAnchorIndex(num gridRowAnchorIndex);

  InstanceNode gridColumnAnchorIndex(num gridColumnAnchorIndex);

  InstanceNode clipsContent(bool clipsContent);

  InstanceNode layoutGrids(List<LayoutGrid> layoutGrids);

  InstanceNode overflowDirection(OverflowDirection overflowDirection);

  InstanceNode layoutMode(LayoutMode layoutMode);

  InstanceNode primaryAxisSizingMode(
    PrimaryAxisSizingMode primaryAxisSizingMode,
  );

  InstanceNode counterAxisSizingMode(
    CounterAxisSizingMode counterAxisSizingMode,
  );

  InstanceNode primaryAxisAlignItems(
    PrimaryAxisAlignItems primaryAxisAlignItems,
  );

  InstanceNode counterAxisAlignItems(
    CounterAxisAlignItems counterAxisAlignItems,
  );

  InstanceNode paddingLeft(num paddingLeft);

  InstanceNode paddingRight(num paddingRight);

  InstanceNode paddingTop(num paddingTop);

  InstanceNode paddingBottom(num paddingBottom);

  InstanceNode itemSpacing(num itemSpacing);

  InstanceNode itemReverseZIndex(bool itemReverseZIndex);

  InstanceNode strokesIncludedInLayout(bool strokesIncludedInLayout);

  InstanceNode layoutWrap(LayoutWrap? layoutWrap);

  InstanceNode counterAxisSpacing(num? counterAxisSpacing);

  InstanceNode counterAxisAlignContent(
    CounterAxisAlignContent counterAxisAlignContent,
  );

  InstanceNode cornerRadius(num cornerRadius);

  InstanceNode cornerSmoothing(num cornerSmoothing);

  InstanceNode rectangleCornerRadii(List<num> rectangleCornerRadii);

  InstanceNode fills(List<Paint> fills);

  InstanceNode styles(Map<String, String> styles);

  InstanceNode strokes(List<Paint> strokes);

  InstanceNode strokeWeight(num strokeWeight);

  InstanceNode strokeAlign(StrokeAlign? strokeAlign);

  InstanceNode strokeJoin(StrokeJoin strokeJoin);

  InstanceNode strokeDashes(List<num> strokeDashes);

  InstanceNode fillOverrideTable(Map<String, PaintOverride?> fillOverrideTable);

  InstanceNode fillGeometry(List<Path> fillGeometry);

  InstanceNode strokeGeometry(List<Path> strokeGeometry);

  InstanceNode strokeCap(StrokeCap strokeCap);

  InstanceNode strokeMiterAngle(num strokeMiterAngle);

  InstanceNode exportSettings(List<ExportSetting> exportSettings);

  InstanceNode effects(List<Effect> effects);

  InstanceNode isMask(bool isMask);

  InstanceNode maskType(MaskType? maskType);

  InstanceNode transitionNodeId(String? transitionNodeId);

  InstanceNode transitionDuration(num? transitionDuration);

  InstanceNode transitionEasing(EasingType? transitionEasing);

  InstanceNode interactions(List<Interaction> interactions);

  InstanceNode individualStrokeWeights(StrokeWeights? individualStrokeWeights);

  InstanceNode devStatus(DevStatus? devStatus);

  InstanceNode componentId(String componentId);

  InstanceNode isExposedInstance(bool isExposedInstance);

  InstanceNode exposedInstances(List<String> exposedInstances);
...
/// Use as `instanceOfInstanceNode.copyWith(...)` or call `instanceOfInstanceNode.copyWith.fieldName(value)` for a single field.
class _$InstanceNodeCWProxyImpl implements _$InstanceNodeCWProxy {
  const _$InstanceNodeCWProxyImpl(this._value);

  final InstanceNode _value;

  @override
  InstanceNode id(String id) => call(id: id);

  @override
  InstanceNode name(String name) => call(name: name);

  @override
  InstanceNode visible(bool visible) => call(visible: visible);

  @override
  InstanceNode locked(bool locked) => call(locked: locked);
  ...
  @override
  InstanceNode rotation(num rotation) => call(rotation: rotation);
  ...
  @override
  InstanceNode pluginData(Object? pluginData) => call(pluginData: pluginData);
  ...
  @override
  InstanceNode explicitVariableModes(
    Map<String, String> explicitVariableModes,
  ) => call(explicitVariableModes: explicitVariableModes);

  @override
  InstanceNode blendMode(BlendMode blendMode) => call(blendMode: blendMode);

  @override
  InstanceNode opacity(num opacity) => call(opacity: opacity);
  ...
  @override
  InstanceNode size(Vector? size) => call(size: size);
  ...
  @override
  InstanceNode minWidth(num minWidth) => call(minWidth: minWidth);

  @override
  InstanceNode maxWidth(num maxWidth) => call(maxWidth: maxWidth);

  @override
  InstanceNode minHeight(num minHeight) => call(minHeight: minHeight);

  @override
  InstanceNode maxHeight(num maxHeight) => call(maxHeight: maxHeight);
  ...
  @override
  InstanceNode gridRowGap(num gridRowGap) => call(gridRowGap: gridRowGap);
  ...
  @override
  InstanceNode gridRowSpan(num gridRowSpan) => call(gridRowSpan: gridRowSpan);
  ...
  @override
  InstanceNode clipsContent(bool clipsContent) =>
      call(clipsContent: clipsContent);
  ...
  @override
  InstanceNode paddingLeft(num paddingLeft) => call(paddingLeft: paddingLeft);
  ...
  @override
  InstanceNode paddingTop(num paddingTop) => call(paddingTop: paddingTop);
  ...
  @override
  InstanceNode itemSpacing(num itemSpacing) => call(itemSpacing: itemSpacing);
  ...
  @override
  InstanceNode fills(List<Paint> fills) => call(fills: fills);

  @override
  InstanceNode styles(Map<String, String> styles) => call(styles: styles);

  @override
  InstanceNode strokes(List<Paint> strokes) => call(strokes: strokes);
  ...
  @override
  InstanceNode fillOverrideTable(
    Map<String, PaintOverride?> fillOverrideTable,
  ...
  @override
  InstanceNode strokeCap(StrokeCap strokeCap) => call(strokeCap: strokeCap);
  ...
  @override
  InstanceNode effects(List<Effect> effects) => call(effects: effects);

  @override
  InstanceNode isMask(bool isMask) => call(isMask: isMask);

  @override
  InstanceNode maskType(MaskType? maskType) => call(maskType: maskType);
  ...
  @override
  InstanceNode individualStrokeWeights(
    StrokeWeights? individualStrokeWeights,
  ) => call(individualStrokeWeights: individualStrokeWeights);

  @override
  InstanceNode devStatus(DevStatus? devStatus) => call(devStatus: devStatus);

  @override
  InstanceNode componentId(String componentId) =>
      call(componentId: componentId);

  @override
  InstanceNode isExposedInstance(bool isExposedInstance) =>
      call(isExposedInstance: isExposedInstance);

  @override
  InstanceNode exposedInstances(List<String> exposedInstances) =>
      call(exposedInstances: exposedInstances);
  ...
  InstanceNode call({
    ...
    return InstanceNode(
      ...
      componentId:
          componentId == const $CopyWithPlaceholder() || componentId == null
          ? _value.componentId
          // ignore: cast_nullable_to_non_nullable
          : componentId as String,
      isExposedInstance:
          isExposedInstance == const $CopyWithPlaceholder() ||
              isExposedInstance == null
          ? _value.isExposedInstance
          // ignore: cast_nullable_to_non_nullable
          : isExposedInstance as bool,
      exposedInstances:
          exposedInstances == const $CopyWithPlaceholder() ||
...
extension $InstanceNodeCopyWith on InstanceNode {
  /// Returns a callable class used to build a new instance with modified fields.
  /// Example: `instanceOfInstanceNode.copyWith(...)` or `instanceOfInstanceNode.copyWith.fieldName(...)`.
  // ignore: library_private_types_in_public_api
  _$InstanceNodeCWProxy get copyWith => _$InstanceNodeCWProxyImpl(this);
}
...
InstanceNode _$InstanceNodeFromJson(Map<String, dynamic> json) => InstanceNode(
  ...
  exposedInstances:
      (json['exposedInstances'] as List<dynamic>?)
          ?.map((e) => e as String)
          .toList() ??
      [],
  componentProperties:
...
Map<String, dynamic> _$InstanceNodeToJson(
