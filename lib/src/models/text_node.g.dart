...
abstract class _$TextNodeCWProxy {
  TextNode id(String id);

  TextNode name(String name);

  TextNode visible(bool visible);

  TextNode locked(bool locked);

  TextNode scrollBehavior(ScrollBehavior scrollBehavior);

  TextNode rotation(num rotation);

  TextNode componentPropertyReferences(
    Map<String, String> componentPropertyReferences,
  );

  TextNode pluginData(Object? pluginData);

  TextNode sharedPluginData(Object? sharedPluginData);

  TextNode boundVariables(LayerTraitVariables boundVariables);

  TextNode explicitVariableModes(Map<String, String> explicitVariableModes);

  TextNode blendMode(BlendMode blendMode);

  TextNode opacity(num opacity);

  TextNode absoluteBoundingBox(Rectangle? absoluteBoundingBox);

  TextNode absoluteRenderBounds(Rectangle? absoluteRenderBounds);

  TextNode preserveRatio(bool preserveRatio);

  TextNode constraints(LayoutConstraint? constraints);

  TextNode relativeTransform(Transform? relativeTransform);

  TextNode size(Vector? size);

  TextNode layoutAlign(LayoutAlign? layoutAlign);

  TextNode layoutGrow(LayoutGrow layoutGrow);

  TextNode layoutPositioning(LayoutPositioning layoutPositioning);

  TextNode minWidth(num minWidth);

  TextNode maxWidth(num maxWidth);

  TextNode minHeight(num minHeight);

  TextNode maxHeight(num maxHeight);

  TextNode layoutSizingHorizontal(LayoutSizing? layoutSizingHorizontal);

  TextNode layoutSizingVertical(LayoutSizing? layoutSizingVertical);

  TextNode gridRowCount(num? gridRowCount);

  TextNode gridColumnCount(num? gridColumnCount);

  TextNode gridRowGap(num gridRowGap);

  TextNode gridColumnGap(num gridColumnGap);

  TextNode gridColumnsSizing(String? gridColumnsSizing);

  TextNode gridRowsSizing(String? gridRowsSizing);

  TextNode gridChildHorizontalAlign(GridChildAlign? gridChildHorizontalAlign);

  TextNode gridChildVerticalAlign(GridChildAlign? gridChildVerticalAlign);

  TextNode gridRowSpan(num gridRowSpan);

  TextNode gridColumnSpan(num gridColumnSpan);

  TextNode gridRowAnchorIndex(num gridRowAnchorIndex);

  TextNode gridColumnAnchorIndex(num gridColumnAnchorIndex);

  TextNode fills(List<Paint> fills);

  TextNode styles(Map<String, String> styles);

  TextNode strokes(List<Paint> strokes);

  TextNode strokeWeight(num strokeWeight);

  TextNode strokeAlign(StrokeAlign? strokeAlign);

  TextNode strokeJoin(StrokeJoin strokeJoin);

  TextNode strokeDashes(List<num> strokeDashes);

  TextNode fillOverrideTable(Map<String, PaintOverride?> fillOverrideTable);

  TextNode fillGeometry(List<Path> fillGeometry);

  TextNode strokeGeometry(List<Path> strokeGeometry);

  TextNode strokeCap(StrokeCap strokeCap);

  TextNode strokeMiterAngle(num strokeMiterAngle);

  TextNode exportSettings(List<ExportSetting> exportSettings);

  TextNode effects(List<Effect> effects);

  TextNode isMask(bool isMask);

  TextNode maskType(MaskType? maskType);

  TextNode transitionNodeId(String? transitionNodeId);

  TextNode transitionDuration(num? transitionDuration);

  TextNode transitionEasing(EasingType? transitionEasing);

  TextNode interactions(List<Interaction> interactions);

  TextNode characters(String characters);

  TextNode style(TypeStyle style);

  TextNode characterStyleOverrides(List<num> characterStyleOverrides);

  TextNode layoutVersion(num? layoutVersion);

  TextNode styleOverrideTable(Map<String, TypeStyle> styleOverrideTable);

  TextNode lineTypes(List<LineType> lineTypes);

  TextNode lineIndentations(List<num> lineIndentations);

  /// Creates a new instance with the provided field values.
  /// Passing `null` to a nullable field nullifies it, while `null` for a non-nullable field is ignored. To update a single field use `TextNode(...).copyWith.fieldName(value)`.
  ///
  /// Example:
  /// ```dart
  ...
  TextNode call({
    ...
    num? layoutVersion,
    Map<String, TypeStyle> styleOverrideTable,
    List<LineType> lineTypes,
    List<num> lineIndentations,
  });
}

/// Callable proxy for `copyWith` functionality.
/// Use as `instanceOfTextNode.copyWith(...)` or call `instanceOfTextNode.copyWith.fieldName(value)` for a single field.
class _$TextNodeCWProxyImpl implements _$TextNodeCWProxy {
  ...
  @override
  TextNode id(String id) => call(id: id);

  @override
  TextNode name(String name) => call(name: name);

  @override
  TextNode visible(bool visible) => call(visible: visible);

  @override
  TextNode locked(bool locked) => call(locked: locked);
  ...
  @override
  TextNode rotation(num rotation) => call(rotation: rotation);
  ...
  @override
  TextNode pluginData(Object? pluginData) => call(pluginData: pluginData);
  ...
  @override
  TextNode blendMode(BlendMode blendMode) => call(blendMode: blendMode);

  @override
  TextNode opacity(num opacity) => call(opacity: opacity);
  ...
  @override
  TextNode size(Vector? size) => call(size: size);
  ...
  @override
  TextNode layoutGrow(LayoutGrow layoutGrow) => call(layoutGrow: layoutGrow);
  ...
  @override
  TextNode minWidth(num minWidth) => call(minWidth: minWidth);

  @override
  TextNode maxWidth(num maxWidth) => call(maxWidth: maxWidth);

  @override
  TextNode minHeight(num minHeight) => call(minHeight: minHeight);

  @override
  TextNode maxHeight(num maxHeight) => call(maxHeight: maxHeight);
  ...
  @override
  TextNode gridRowCount(num? gridRowCount) => call(gridRowCount: gridRowCount);
  ...
  @override
  TextNode gridRowGap(num gridRowGap) => call(gridRowGap: gridRowGap);
  ...
  @override
  TextNode gridRowSpan(num gridRowSpan) => call(gridRowSpan: gridRowSpan);
  ...
  @override
  TextNode fills(List<Paint> fills) => call(fills: fills);

  @override
  TextNode styles(Map<String, String> styles) => call(styles: styles);

  @override
  TextNode strokes(List<Paint> strokes) => call(strokes: strokes);

  @override
  TextNode strokeWeight(num strokeWeight) => call(strokeWeight: strokeWeight);
  ...
  @override
  TextNode strokeJoin(StrokeJoin strokeJoin) => call(strokeJoin: strokeJoin);
  ...
  @override
  TextNode strokeCap(StrokeCap strokeCap) => call(strokeCap: strokeCap);
  ...
  @override
  TextNode effects(List<Effect> effects) => call(effects: effects);

  @override
  TextNode isMask(bool isMask) => call(isMask: isMask);

  @override
  TextNode maskType(MaskType? maskType) => call(maskType: maskType);
  ...
  @override
  TextNode characters(String characters) => call(characters: characters);

  @override
  TextNode style(TypeStyle style) => call(style: style);
  ...
  @override
  TextNode lineTypes(List<LineType> lineTypes) => call(lineTypes: lineTypes);

  @override
  TextNode lineIndentations(List<num> lineIndentations) =>
      call(lineIndentations: lineIndentations);
  ...
  TextNode call({
    ...
    Object? lineTypes = const $CopyWithPlaceholder(),
    Object? lineIndentations = const $CopyWithPlaceholder(),
  }) {
    return TextNode(
      ...
      styleOverrideTable:
          ...
          // ignore: cast_nullable_to_non_nullable
          : styleOverrideTable as Map<String, TypeStyle>,
      lineTypes: lineTypes == const $CopyWithPlaceholder() || lineTypes == null
          ? _value.lineTypes
          // ignore: cast_nullable_to_non_nullable
          : lineTypes as List<LineType>,
      lineIndentations:
          lineIndentations == const $CopyWithPlaceholder() ||
              lineIndentations == null
          ? _value.lineIndentations
          // ignore: cast_nullable_to_non_nullable
          : lineIndentations as List<num>,
    );
  }
}
...
extension $TextNodeCopyWith on TextNode {
  ...
  /// Example: `instanceOfTextNode.copyWith(...)` or `instanceOfTextNode.copyWith.fieldName(...)`.
...
TextNode _$TextNodeFromJson(Map<String, dynamic> json) => TextNode(
  ...
  style: TypeStyle.fromJson(json['style'] as Map<String, dynamic>),
  ...
  styleOverrideTable: (json['styleOverrideTable'] as Map<String, dynamic>).map(
    (k, e) => MapEntry(k, TypeStyle.fromJson(e as Map<String, dynamic>)),
  ),
  lineTypes: (json['lineTypes'] as List<dynamic>)
...
Map<String, dynamic> _$TextNodeToJson(TextNode instance) => <String, dynamic>{
  ...
  'lineTypes': instance.lineTypes.map((e) => _$LineTypeEnumMap[e]!).toList(),
...
const _$LineTypeEnumMap = {
  LineType.none: 'NONE',
  LineType.ordered: 'ORDERED',
  LineType.unordered: 'UNORDERED',
};
