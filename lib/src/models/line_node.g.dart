...
part of 'line_node.dart';

// **************************************************************************
// CopyWithGenerator
// **************************************************************************

abstract class _$LineNodeCWProxy {
  LineNode id(String id);

  LineNode name(String name);

  LineNode visible(bool visible);

  LineNode locked(bool locked);

  LineNode scrollBehavior(ScrollBehavior scrollBehavior);

  LineNode rotation(num rotation);

  LineNode componentPropertyReferences(
    Map<String, String> componentPropertyReferences,
  );

  LineNode pluginData(Object? pluginData);

  LineNode sharedPluginData(Object? sharedPluginData);

  LineNode boundVariables(LayerTraitVariables boundVariables);

  LineNode explicitVariableModes(Map<String, String> explicitVariableModes);

  LineNode blendMode(BlendMode blendMode);

  LineNode opacity(num opacity);

  LineNode absoluteBoundingBox(Rectangle? absoluteBoundingBox);

  LineNode absoluteRenderBounds(Rectangle? absoluteRenderBounds);

  LineNode preserveRatio(bool preserveRatio);

  LineNode constraints(LayoutConstraint? constraints);

  LineNode relativeTransform(Transform? relativeTransform);

  LineNode size(Vector? size);

  LineNode layoutAlign(LayoutAlign? layoutAlign);

  LineNode layoutGrow(LayoutGrow layoutGrow);

  LineNode layoutPositioning(LayoutPositioning layoutPositioning);

  LineNode minWidth(num minWidth);

  LineNode maxWidth(num maxWidth);

  LineNode minHeight(num minHeight);

  LineNode maxHeight(num maxHeight);

  LineNode layoutSizingHorizontal(LayoutSizing? layoutSizingHorizontal);

  LineNode layoutSizingVertical(LayoutSizing? layoutSizingVertical);

  LineNode gridRowCount(num? gridRowCount);

  LineNode gridColumnCount(num? gridColumnCount);

  LineNode gridRowGap(num gridRowGap);

  LineNode gridColumnGap(num gridColumnGap);

  LineNode gridColumnsSizing(String? gridColumnsSizing);

  LineNode gridRowsSizing(String? gridRowsSizing);

  LineNode gridChildHorizontalAlign(GridChildAlign? gridChildHorizontalAlign);

  LineNode gridChildVerticalAlign(GridChildAlign? gridChildVerticalAlign);

  LineNode gridRowSpan(num gridRowSpan);

  LineNode gridColumnSpan(num gridColumnSpan);

  LineNode gridRowAnchorIndex(num gridRowAnchorIndex);

  LineNode gridColumnAnchorIndex(num gridColumnAnchorIndex);

  LineNode fills(List<Paint> fills);

  LineNode styles(Map<String, String> styles);

  LineNode strokes(List<Paint> strokes);

  LineNode strokeWeight(num strokeWeight);

  LineNode strokeAlign(StrokeAlign? strokeAlign);

  LineNode strokeJoin(StrokeJoin strokeJoin);

  LineNode strokeDashes(List<num> strokeDashes);

  LineNode fillOverrideTable(Map<String, PaintOverride?> fillOverrideTable);

  LineNode fillGeometry(List<Path> fillGeometry);

  LineNode strokeGeometry(List<Path> strokeGeometry);

  LineNode strokeCap(StrokeCap strokeCap);

  LineNode strokeMiterAngle(num strokeMiterAngle);

  LineNode exportSettings(List<ExportSetting> exportSettings);

  LineNode effects(List<Effect> effects);

  LineNode isMask(bool isMask);

  LineNode maskType(MaskType? maskType);

  LineNode transitionNodeId(String? transitionNodeId);

  LineNode transitionDuration(num? transitionDuration);

  LineNode transitionEasing(EasingType? transitionEasing);
...
/// Use as `instanceOfLineNode.copyWith(...)` or call `instanceOfLineNode.copyWith.fieldName(value)` for a single field.
class _$LineNodeCWProxyImpl implements _$LineNodeCWProxy {
  const _$LineNodeCWProxyImpl(this._value);

  final LineNode _value;

  @override
  LineNode id(String id) => call(id: id);

  @override
  LineNode name(String name) => call(name: name);

  @override
  LineNode visible(bool visible) => call(visible: visible);

  @override
  LineNode locked(bool locked) => call(locked: locked);
  ...
  @override
  LineNode rotation(num rotation) => call(rotation: rotation);
  ...
  @override
  LineNode pluginData(Object? pluginData) => call(pluginData: pluginData);
  ...
  @override
  LineNode blendMode(BlendMode blendMode) => call(blendMode: blendMode);

  @override
  LineNode opacity(num opacity) => call(opacity: opacity);
  ...
  @override
  LineNode size(Vector? size) => call(size: size);
  ...
  @override
  LineNode layoutGrow(LayoutGrow layoutGrow) => call(layoutGrow: layoutGrow);
  ...
  @override
  LineNode minWidth(num minWidth) => call(minWidth: minWidth);

  @override
  LineNode maxWidth(num maxWidth) => call(maxWidth: maxWidth);

  @override
  LineNode minHeight(num minHeight) => call(minHeight: minHeight);

  @override
  LineNode maxHeight(num maxHeight) => call(maxHeight: maxHeight);
  ...
  @override
  LineNode gridRowCount(num? gridRowCount) => call(gridRowCount: gridRowCount);
  ...
  @override
  LineNode gridRowGap(num gridRowGap) => call(gridRowGap: gridRowGap);
  ...
  @override
  LineNode gridRowSpan(num gridRowSpan) => call(gridRowSpan: gridRowSpan);
  ...
  @override
  LineNode fills(List<Paint> fills) => call(fills: fills);

  @override
  LineNode styles(Map<String, String> styles) => call(styles: styles);

  @override
  LineNode strokes(List<Paint> strokes) => call(strokes: strokes);

  @override
  LineNode strokeWeight(num strokeWeight) => call(strokeWeight: strokeWeight);
  ...
  @override
  LineNode strokeJoin(StrokeJoin strokeJoin) => call(strokeJoin: strokeJoin);
  ...
  @override
  LineNode strokeCap(StrokeCap strokeCap) => call(strokeCap: strokeCap);
  ...
  @override
  LineNode effects(List<Effect> effects) => call(effects: effects);

  @override
  LineNode isMask(bool isMask) => call(isMask: isMask);

  @override
  LineNode maskType(MaskType? maskType) => call(maskType: maskType);
...
extension $LineNodeCopyWith on LineNode {
  /// Returns a callable class used to build a new instance with modified fields.
  /// Example: `instanceOfLineNode.copyWith(...)` or `instanceOfLineNode.copyWith.fieldName(...)`.
  // ignore: library_private_types_in_public_api
  _$LineNodeCWProxy get copyWith => _$LineNodeCWProxyImpl(this);
}
...
LineNode _$LineNodeFromJson(Map<String, dynamic> json) => LineNode(
...
Map<String, dynamic> _$LineNodeToJson(LineNode instance) => <String, dynamic>{
  'id': instance.id,
  'name': instance.name,
